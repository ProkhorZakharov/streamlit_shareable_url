...
def patched(f, _globals = {}):
    ...
    cd = f.__code__
    _new_globals = {
        k :  f.__globals__[k] for k in _extract_code_globals(cd) if k in  f.__globals__ #mod.__dict__
    }
...
def shareable_wrapper(
    ...
    # Check if layout is given in px
    actual_layout = layout
    if layout.endswith('px'): 
        actual_layout = 'centered'
    ...
    def _func(**kwargs):
        ...
        if True:
            ...
            # Patch with session state
            
            for k in defaults.keys():
                if k not in query_params and k in st.session_state:
                    query_params[k] = [st.session_state[k]]
